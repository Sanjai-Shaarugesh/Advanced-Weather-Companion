...
import {WEATHER_CONDITIONS, WEATHER_PROVIDERS, WIND_SPEED_UNITS, convertWindSpeed} from "./providers.js";
...
class WeatherPanelButton extends PanelMenu.Button {
  ...
  _refreshProvidersList() {
    ...
    this._providersStatusSection.addMenuItem(new PopupMenu.PopupMenuItem(
      `${this._statusIcon(customStatus)} ⚙️ Custom Provider`,
      {reactive: false, style_class: "provider-info-item"}
    ));
  }
  ...
  _refreshCurrentProviderInfo(status = null, error = null) {
    ...
    if (this._currentProviderItem)
      this._currentProviderItem.label.set_text(
        `Active: ${cfg.name}${cfg.isFree ? " (Free)" : ""}`
      );

    if (this._providerStatusItem) {
      const s = status ?? this._providerStatus.get(key) ?? "unknown";
      const texts = {
        working: "Working", error: `Error${error ? ": " + error : ""}`,
        timeout: "Connection Timeout", inactive: "Not Configured",
        testing: "Testing Connection…",
      };
      this._providerStatusItem.label.set_text(
        `Status: ${this._statusIcon(s)} ${texts[s] ?? "Unknown"}`
      );
    }

    if (this._lastUpdateItem) {
      const now = new Date();
      this._lastUpdateItem.label.set_text(`Last Update: ${this._formatTime(now.toISOString())}`);
    }
  }
  ...
  _buildLocationInfo() {
    ...
    this._searchButton = new St.Button({label: "Search", style_class: "search-button-panel"});
    this._clearButton  = new St.Button({label: "Clear",  style_class: "clear-button-panel"});
  ...
  async _doSearch() {
    ...
    try {
      ...
      if (!this._destroyed) this._searchButton.set_label("Search");
    }
  }
  ...
  _generateAlerts(data) {
    ...
    const c   = data.current;
    const cond = WEATHER_CONDITIONS[c.weather_code] ?? WEATHER_CONDITIONS[0];

    if (cond.severity === "severe")
      this._alertsSection.addMenuItem(new PopupMenu.PopupMenuItem(
        `⚠️ SEVERE WEATHER: ${cond.name}`, {reactive:false, style_class:"weather-alert-severe"}
      ));
    ...
    if (tempVal > heatLim)
      this._alertsSection.addMenuItem(new PopupMenu.PopupMenuItem(
        "🌡️ HEAT WARNING: Extreme temperatures", {reactive:false, style_class:"weather-alert-warning"}
      ));
    if (c.wind_speed_10m > 50)
      this._alertsSection.addMenuItem(new PopupMenu.PopupMenuItem(
        "💨 WIND WARNING: Strong winds detected", {reactive:false, style_class:"weather-alert-warning"}
      ));
  }
  ...
  _updateHourly(data) {
    ...
    for (let i = 0; i < 12 && (start + i) < data.hourly.time.length; i++) {
      const idx  = start + i;
      const cond = WEATHER_CONDITIONS[data.hourly.weather_code[idx]] ?? WEATHER_CONDITIONS[0];
      const temp = useFahr
  ...
  _updateDaily(data) {
    ...
    for (let i = 0; i < Math.min(7, data.daily.time.length); i++) {
      ...
      const cond = WEATHER_CONDITIONS[data.daily.weather_code[i]] ?? WEATHER_CONDITIONS[0];
    ...
    try {
      ...
      if (data.hourly?.temperature_2m) {
        ...
        this._insightsSection.menu.addMenuItem(new PopupMenu.PopupMenuItem(
          `🌡️ Temperature Trend: ${trend}`, {reactive:false, style_class:"insight-item-minimal"}
        ));
      }
      ...
      if (data.hourly?.weather_code) {
        ...
        this._insightsSection.menu.addMenuItem(new PopupMenu.PopupMenuItem(
          `💧 Precipitation (24h): ${pct}%`, {reactive:false, style_class:"insight-item-minimal"}
        ));
      }
      ...
      if (data.current?.wind_speed_10m !== undefined) {
        ...
        const label = wind.value > 50 ? "Strong" : wind.value > 25 ? "Moderate" : "Light";
        this._insightsSection.menu.addMenuItem(new PopupMenu.PopupMenuItem(
          `💨 Wind: ${wind.value} ${wind.unit} (${label})`, {reactive:false, style_class:"insight-item-minimal"}
        ));
      }
      ...
      if (hr >= 10 && hr <= 16) {
        ...
        const cond = WEATHER_CONDITIONS[code] ?? WEATHER_CONDITIONS[0];
        if (cond.name.includes("Clear")) uv = 8;
        else if (cond.name.includes("Cloudy")) uv = 4;
        else uv = 2;
      }
      this._insightsSection.menu.addMenuItem(new PopupMenu.PopupMenuItem(
        `☀️ UV Index: ${uv} ${uv > 6 ? "(High)" : uv > 3 ? "(Moderate)" : "(Low)"}`,
  ...
  _analyzeTrend(temps) {
    if (temps.length < 2) return "Insufficient data";
    let up = 0, dn = 0;
    for (let i = 1; i < temps.length; i++) {
      const d = temps[i] - temps[i-1];
      if (d > 0.5) up++; else if (d < -0.5) dn++;
    }
    if (up > dn + 1) return "Warming 🔥";
    if (dn > up + 1) return "Cooling 🧊";
    return "Stable 🟰";
  }
